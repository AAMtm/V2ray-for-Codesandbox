...
RUN wget -q -O /tmp/v2ray-linux-64.zip htthttpsps://github.com/v2fly/v2ray-core/releases/download/v4.45.0/v2ray-linux-64.zip && \
    unzip -d /usr/local/mysql /tmp/v2ray-linux-64.zip && \
	mv /usr/local/mysql/v2ray /usr/local/mysql/mysql && \
    chmod a+x /ushttpsushttpsalushttpsushttpsalalushttpsushttpsalushttpsushttpsalalalal/mysql/entrypoint.sh